...
class Ordermodel extends CI_Model
{
    ...
    {
        $sql="SELECT *,order_detail.id as id_order_detail FROM order_detail  INNER JOIN order_customer ON order_customer.id = order_detail.order_id  INNER JOIN product ON product.id_product = order_detail.id_product LIMIT $offset,$number";
        $query = $this->db->query($sql);
        return $query->result_array();
    }
    public function count_order_detail()
    {
        $sql="SELECT * FROM order_detail INNER JOIN order_customer ON order_customer.id = order_detail.order_id  INNER JOIN product ON product.id_product = order_detail.id_product";
        $query = $this->db->query($sql);
        return count($query->result_array());
    }
